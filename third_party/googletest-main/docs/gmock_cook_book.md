...
matcher can never call a mock function, as that will affect the shadowState of the
...
effects. For example, it may change some global shadowState or modify an output
...
internal shadowState (i.e. if it always does the same thing no matter how many times
...
However, if the action has its own shadowState, you may be surprised if you share the
